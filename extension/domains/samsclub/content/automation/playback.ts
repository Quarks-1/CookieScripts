...
} from "@ext/domains/samsclub/lib/cart-step.ts";
import { retryUntilConfirmed } from "@ext/domains/samsclub/lib/cart-retry.ts";
import {
...
import { ensurePageQuantityBeforeAtc } from "@ext/domains/samsclub/lib/quantity-limit.ts";
import {
...
import { runPlaybackEngine, resolveCheckoutStartUrl } from "@ext/domains/samsclub/lib/playback-engine.ts";
import { CHECKOUT_START_URL, DEFAULT_ADD_TO_CART_SELECTORS } from "@ext/domains/samsclub/lib/selectors.ts";
...
export async function runAutomationPlayback(
  ...
  const addResult = await runPlaybackEngine(playbackSteps, {
    ...
    navigate: (url) => {
      ...
      const target =
        url === CHECKOUT_START_URL || url.startsWith("https://www.samsclub.com/checkout")
          ? resolveCheckoutStartUrl(document)
          : url;
