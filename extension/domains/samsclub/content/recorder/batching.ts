import { BATCH_FLUSH_MS, BATCH_MAX_EVENTS } from "@ext/domains/samsclub/lib/constants.ts";
import { estimateBytes } from "@ext/domains/samsclub/lib/recording-limits.ts";
import { sendToBackground } from "@ext/core/lib/messages.ts";
import type { BackgroundResponse } from "@ext/core/types/index.ts";
import type { PageSnapshotRecord, SamsclubRecordingEvent } from "@ext/domains/samsclub/types/samsclub.ts";
...
export class RecordingBatcher {
  private events: SamsclubRecordingEvent[] = [];
  ...
  enqueue(event: SamsclubRecordingEvent, byteDelta = 0): void {
  ...
  enqueuePage(event: SamsclubRecordingEvent, page: PageSnapshotRecord, byteDelta: number): void {
    this.events.push(event);
