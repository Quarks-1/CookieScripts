import { graphqlSignalForOperation, isOrchestraFailureStatus } from "@ext/domains/samsclub/lib/graphql-signals.ts";
import { redactBodySnippet, redactHeaders } from "@ext/domains/samsclub/lib/network-redact.ts";
import {
  addNetworkBytes,
  estimateBytes,
  networkBodyLimit,
  type SessionLimitState,
} from "@ext/domains/samsclub/lib/recording-limits.ts";
import { parseGraphqlOperation, redactUrl, resolveAbsoluteUrl } from "@ext/domains/samsclub/lib/url.ts";
...
export function probeDetailToEvent(
...
): { event: SamsclubRecordingEvent | null; limitState: SessionLimitState } {
  ...
  if (detail.kind === "websocket") {
    const event: SamsclubRecordingEvent = {
      kind: "websocket",
      ts,
      correlationId:
