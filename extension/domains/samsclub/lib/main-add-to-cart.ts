...
const STICKY_SCOPE_SELECTOR = '[class*="sticky"]';
...
/** @deprecated Use parseSamsclubItemIdFromUrl */
export function parseTargetTcinFromUrl(url: string): string | null {
  return parseSamsclubItemIdFromUrl(url);
}
...
function isNonAtcButton(element: HTMLElement): boolean {
  ...
  if (FIND_ALTERNATIVE_TEXT.test(text)) {
    return true;
  }

  if (element.getAttribute("data-automation-id") === "atc") {
    const aria = element.getAttribute("aria-label") ?? "";
    if (CAROUSEL_ATC_ARIA.test(aria) && !ADD_TO_CART_TEXT.test(aria)) {
      return true;
    }
  }
