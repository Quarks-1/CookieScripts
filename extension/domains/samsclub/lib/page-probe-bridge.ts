import {
  SAMSCLUB_PROBE_BRIDGE_ID,
  SAMSCLUB_PROBE_EVENT,
  SAMSCLUB_PROBE_SCRIPT_PATH,
...
function defaultGetScriptUrl(): string {
  return chrome.runtime.getURL(SAMSCLUB_PROBE_SCRIPT_PATH);
}
...
function injectBridgeScript(
  ...
  script.id = SAMSCLUB_PROBE_BRIDGE_ID;
...
export async function ensureSamsclubResearchProbe(
  doc: Document,
  getScriptUrl: () => string = defaultGetScriptUrl,
): Promise<"ready" | "failed"> {
  const existing = doc.getElementById(SAMSCLUB_PROBE_BRIDGE_ID);
...
export function removeSamsclubResearchProbe(doc: Document): void {
  const existing = doc.getElementById(SAMSCLUB_PROBE_BRIDGE_ID);
  existing?.remove();
}

export function listenSamsclubProbe(
  doc: Document,
  ...
  doc.addEventListener(SAMSCLUB_PROBE_EVENT, handler);
  return () => doc.removeEventListener(SAMSCLUB_PROBE_EVENT, handler);
}

export { SAMSCLUB_PROBE_EVENT };
