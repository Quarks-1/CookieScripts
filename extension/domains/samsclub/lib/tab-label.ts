...
export function cleanSamsclubProductTitle(title: string): string {
...
function labelForKind(kind: SamsclubPageKind, url: string, title: string): string {
  switch (kind) {
    case "home":
      return "Home";
    case "cart":
    ...
    case "product": {
      const cleaned = cleanSamsclubProductTitle(title);
      if (cleaned) {
        return cleaned;
      }
...
export function labelSamsclubTab(
  url: string,
  title?: string,
): { label: string; pageKind: SamsclubPageKind } {
  const pageKind = classifySamsclubPageKind(url);
