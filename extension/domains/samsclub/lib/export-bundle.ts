...
import { markerSlug } from "@ext/domains/samsclub/lib/endpoint-index.ts";
import { listEndpoints } from "@ext/domains/samsclub/lib/idb/endpoints-store.ts";
import { listEvents } from "@ext/domains/samsclub/lib/idb/event-store.ts";
import { listPages } from "@ext/domains/samsclub/lib/idb/pages-store.ts";
import { getSession } from "@ext/domains/samsclub/lib/idb/session-store.ts";
import type { SamsclubRecordingEvent, SamsclubSessionMeta } from "@ext/domains/samsclub/types/samsclub.ts";

function isNetworkEvent(
  event: SamsclubRecordingEvent,
): event is Extract<SamsclubRecordingEvent, { kind: "network" | "websocket" }> {
  return event.kind === "network" || event.kind === "websocket";
}

function buildSummary(meta: SamsclubSessionMeta, events: SamsclubRecordingEvent[]) {
...
export async function buildExportZip(sessionId: string): Promise<{
  ...
  const base = `CookieScripts/samsclub-live/${meta.dropDate}/session-${meta.sessionTime}`;
