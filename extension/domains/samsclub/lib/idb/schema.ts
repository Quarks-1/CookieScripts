import { SAMSCLUB_DB_NAME, SAMSCLUB_DB_VERSION } from "@ext/domains/samsclub/lib/constants.ts";
...
export function openSamsclubDb(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(SAMSCLUB_DB_NAME, SAMSCLUB_DB_VERSION);
...
export async function withSamsclubDb<T>(fn: (db: IDBDatabase) => Promise<T>): Promise<T> {
  const db = await openSamsclubDb();
  try {
    return await fn(db);
  } finally {
    db.close();
...
export async function idbGet<T>(storeName: string, key: IDBValidKey): Promise<T | undefined> {
  return withSamsclubDb(async (db) => {
