...
export function patchWalmartOpenTabActive<T extends { tabId: number; isActive: boolean }>(
  openTabs: readonly T[],
...
export function isWalmartOpenTabActive(
  tabId: number,
...
export function applyWalmartOpenTabHighlights<T extends { tabId: number; isActive: boolean }>(
  openTabs: readonly T[],
  focusedActiveTabId?: number,
): T[] {
  ...
  return patchWalmartOpenTabActive(openTabs, focusedActiveTabId);
}
