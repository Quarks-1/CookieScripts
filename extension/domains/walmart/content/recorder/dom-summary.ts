import type { DomButtonSummary } from "@ext/domains/walmart/types/walmart.ts";
import { scanDomSignals } from "@ext/domains/walmart/content/recorder/dom-signals.ts";
...
export function readStorageKeyNames(): { local: string[]; session: string[] } {
  ...
  try {
    for (let i = 0; i < sessionStorage.length; i += 1) {
      ...
      if (key && key !== "cookiescripts:walmartRecordingSessionId") {
        session.push(key);
      }
    }
  } catch {
